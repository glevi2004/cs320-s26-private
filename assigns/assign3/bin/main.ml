...
let process_repl user_input env =
  match Assign3.interp user_input env with
  | output, env -> Printf.printf "- : int = %d\n" output; env
  | exception _ -> print_endline "whoops!"; env

let process_file user_input env =
  match Assign3.interp user_input env with
  | output, _ -> Printf.printf "%d\n" output
  | exception _ -> print_endline "whoops!"

let welcome_msg = "ArithVar version 0.0.1\nEnter #quit;; to exit\n"
let usage_str = "usage: dune exec assign3 <filename>"
